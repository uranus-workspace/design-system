...
## Subcomponentes (polimento)

O root continua com os slots **`breadcrumbs`**, **`title`**, **`description`** e **`actions`**. Para layouts totalmente customizados, use os mesmos elementos com **`PageHeader.Breadcrumbs`**, **`.Title`**, **`.Description`** e **`.Actions`** (mesmas classes e `data-slot`).
